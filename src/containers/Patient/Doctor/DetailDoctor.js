...
//         return (
            ...
            //                                     <span className='after-sale'>{detailItem && detailItem.price ? detailItem.price : ''} ₫</span>
...
const DetailDoctor = (props) => {
    ...
    const getResponse = async () => {
        const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}api/get-detail-items-by-id?id=${props.match.params.id}`)
            .then(res => res.json());
        const item = res.data
        setDetailItem(await item);
    }
                    ...
                    <div className='content-left col-8'>
                       <div className='item col-8 row'>
                            ...
                            <div className='text col-6'>
                                ...
                                <div className='price'>
                                        <span 
                                            className={detailItem && detailItem.priceBeforeSale === '' ? 'before-sale-none' : 'before-sale'}
                                        >
                                                {detailItem && detailItem.priceBeforeSale ? detailItem.priceBeforeSale : ''} ₫
                                        </span>
                                        <span 
                                            className='after-sale'
                                            >
                                                {detailItem && detailItem.price ? detailItem.price : ''} ₫
                                        </span>
                                        <div 
                                            className='long-text'
                                        >                    
                                    ...
                                    <div className='buying'>
                                        <input className="form-control node col-3" type="number"
                                            // value="1"
                                        />
                                        <button className='btn-buying' onClick={() => GoToCart(detailItem)}>Đặt mua</button>
                                    </div>
                                    <div className='socal-media'>
                                        <i className="fa-brands fa-square-facebook"></i>
                                        <i className="fa-brands fa-instagram"></i>  
                       ...
                       <div className='detail col-8 row'>
                            <div className='detial-comment col-12'>
                                <div className='detial-title'>
                                    <h2>CHI TIẾT SẢN PHẨM</h2>
                                    <h2>BÌNH LUẬN</h2>
                                </div>
                                <div className='detial-content'>
                                    Kể từ ngày GS.TSKH Bùi Quốc Châu 
                                </div>
                            </div>
                            <div className='new-item col-12'>             
                                {/* <SimilarProduct settings={settings} itemsRedux={itemsRedux}/> */}
                                <NewProduct settings={settings}  />
                                <BestSaleProduct settings={settings}  />
                            </div>
